...
sub find_ms
{
    ...
    if($iso)
    {
        foreach $i (0..($self->{Num}-1))
        {
            if ($s->{'Platform'} == 0)
            {
                if(!$self->{' mstable'} || $self->{' mstable'}->{'Platform'} != 0)
                {
                    $self->{' mstable'} = $s;
                }
                else
                {
                    $self->{' mstable'} = $s if($s->{'Encoding'} >= $self->{' mstable'}->{'Encoding'});
                }
            }
        }
    }
...
sub out
{
    ...
    for ($i = 0; $i < $self->{'Num'}; $i++)
    {
        ...
        } elsif ($s->{'Format'} == 4)
        {
            ...
            ($num, $sRange, $eSel, $eShift) = PDF::API2::Basic::TTF::Utils::TTF_bininfo($num, 2);

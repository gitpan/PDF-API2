...
#
#
#   $Id: GSUB.pm,v 1.3 2003/12/08 13:05:23 Administrator Exp $
#
#=======================================================================
package PDF::API2::Basic::TTF::GSUB;
...
The structure of a GSUB table is the same as that given in L<PDF::API2::Basic::TTF::Ttopen>.
...
sub out_sub
{
    ...
    if ($type == 1)
    {
        $out = pack("nn", $fmt, PDF::API2::Basic::TTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 2));
        if ($fmt == 1)
    ...
    PDF::API2::Basic::TTF::Ttopen::out_final($fh, $out, [[$ctables, 0]]);
    $lookup;
}

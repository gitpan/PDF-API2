package PDF::API2::PDF::File;

=head1 NAME

PDF::API2::PDF::File - Holds the trailers and cross-reference tables for a PDF file

=head1 SYNOPSIS

 $p = PDF::API2::PDF::File->open("filename.pdf", 1);
 $p->new_obj($obj_ref);
 $p->free_obj($obj_ref);
 $p->append_file;
 $p->close_file;
...
dictionary. In the case of a PDF::API2::PDF::File, the elements reflect those in the
trailer dictionary.
...
=head2 $p = PDF::API2::PDF::File->open($filename, $update)
...
the C<PDF::API2::PDF::File> object (so if you need to save, be sure to do it before
calling this method).  

B<NOTE>, that it is important that you call this method on any
C<PDF::API2::PDF::File> object when you wish to destruct it and free up its memory.
...
able to do anything else with the C<PDF::API2::PDF::File> object; it'll have B<no>
internal state whatsoever.

B<Developer note:> As part of the brute-force cleanup done here, this method
will throw a warning message whenever unexpected key values are found within
the C<PDF::API2::PDF::File> object.  This is done to help ensure that any unexpected
...
sub readval
{
    ...
    } elsif ($str =~ m|^/($reg_char+)|so)        # name
    {
        ...
        $value = $1;
        $str =~ s|^/($reg_char+)||so;
        $res = PDF::API2::PDF::Name->from_pdf($value, $self);
    ...
    } elsif ($str =~ m/^(true|false)$irreg_char/o)                        # boolean
    {
        $value = $1;
        $str =~ s/^(?:true|false)//o;
        $res = PDF::API2::PDF::Bool->from_pdf($value);
    } elsif ($str =~ m/^([+-.0-9]+)$irreg_char/o)                             # number
    {
        $value = $1;
        $str =~ s/^([+-.0-9]+)//o;
        $res = PDF::API2::PDF::Number->from_pdf($value);
...
sub new_obj
{
            ...
            if (!defined $self->locate_obj($i, $ng))
            {
                ...
                else
                {
                    $res = $self->test_obj($i, $ng)
                            || $self->add_obj(PDF::API2::PDF::Objind->new(), $i, $ng);
    ...
    {
        $res = $self->add_obj(PDF::API2::PDF::Objind->new(), $i, 0);
        $self->out_obj($res);
        return $res;
    }
}
...
# test for linearized and remove it from output
    ...
    if ($obj->isa('PDF::API2::PDF::Dict') && $obj->{'Linearized'})
    { $out->free_obj($obj); }

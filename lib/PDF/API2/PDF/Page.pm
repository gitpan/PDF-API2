...
PDF::API2::PDF::Page - Represents a PDF page, inherits from L<PDF::API2::PDF::Pages>
...
=head2 PDF::API2::PDF::Page->new($pdf, $parent, $index)
...
sub add
{
    ...
    if (!defined $strm)
    {
        $strm = PDF::API2::PDF::Dict->new;
        foreach (@{$self->{' outto'}})
        { $_->new_obj($strm); }
        $self->{'Contents'} = PDFArray() unless defined $self->{'Contents'};
        unless (ref $self->{'Contents'} eq "PDF::API2::PDF::Array")

package PDF::API2::PDF::TTFont0;

=head1 NAME

PDF::API2::PDF::TTFont0 - Inherits from L<PDF::Dict> and represents a TrueType Type 0
...
#        $ff2->{' stream'} = "";
#        $ffh = PDF::API2::PDF::TTIOString->new(\$ff2->{' stream'});
...
{
    my ($self, $text) = @_;
    my (@clist) = PDF::API2::PDF::Utils::unpacku($text);
    my ($f) = $self->{' font'};
    my ($g, $res);

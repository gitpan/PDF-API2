package PDF::API2::TTF::Anchor;

=head1 TITLE

PDF::API2::TTF::Anchor - Anchor points for GPOS tables
...
sub read
{
    ...
    } elsif ($fmt == 3)
    {
        ...
        {
            $fh->seek($loc + $xoff, 0);
            $self->{'xdev'} = PDF::API2::TTF::Delta->new->read($fh);
        }
        if ($yoff)
        {
            $fh->seek($loc + $yoff, 0);
            $self->{'ydev'} = PDF::API2::TTF::Delta->new->read($fh);
        }
    }
    $self;
}

package PDF::API2::TTF::Fpgm;

=head1 NAME

PDF::API2::TTF::Fpgm - Font program in a TrueType font. Called when a font is loaded
...
sub out_xml
{
    ...
    $self->read;
    $dat = PDF::API2::TTF::Utils::XML_binhint($self->{' dat'});
...
sub XML_end
{
    ...
    if ($tag eq 'code')
    {
        $self->{' dat'} = PDF::API2::TTF::Utils::XML_hintbin($context->{'text'});
        return $context;
    } else

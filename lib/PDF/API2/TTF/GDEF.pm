package PDF::API2::TTF::GDEF;

=head1 NAME

PDF::API2::TTF::Gdef - Opentype GDEF table support
...
This is an L<PDF::API2::TTF::Coverage> Class Definition table containing information
...
sub read
{
    ...
    {
        $fh->seek($goff + $boff, 0);
        $self->{'GLYPH'} = PDF::API2::TTF::Coverage->new(0)->read($fh);
    }

    if ($new_gdef && $moff > 0)
    {
        $fh->seek($moff + $boff, 0);
        $self->{'MARKS'} = PDF::API2::TTF::Coverage->new(0)->read($fh);
    }
    ...
    if ($aoff > 0)
    {
        ...
        $self->{'ATTACH'}{'COVERAGE'} = PDF::API2::TTF::Coverage->new(1)->read($fh);
    ...
    if ($loff > 0)
    {
        ...
        $self->{'LIG'}{'COVERAGE'} = PDF::API2::TTF::Coverage->new(1)->read($fh);
            ...
            foreach $s (TTF_Unpack('S*', $dat))
            {
                ...
                if ($comp->{'FMT'} == 3)
                {
                    ...
                    else
                    {
                        ...
                        $comp->{'DEVICE'} = PDF::API2::TTF::Delta->new->read($fh);
...
sub out
{
    ...
    if (defined $self->{'LIG'})
    {
        ...
        $loff = $fh->tell() - $loc;
        $out = pack('n*',
                        PDF::API2::TTF::Ttopen->ref_cache($self->{'LIG'}{'COVERAGE'}, $ltables, 0),
                        0, $#{$self->{'LIG'}{'LIGS'}} + 1,
                        (0) x ($#{$self->{'LIG'}{'LIGS'}} + 1));
        ...
        foreach $r (@{$self->{'LIG'}{'LIGS'}})
        {
            ...
            foreach $s (@$r)
            {
                ...
                $out .= pack('n', PDF::API2::TTF::Ttopen->ref_cache($s->{'DEVICE'},

package PDF::API2::TTF::GPOS;

=head1 TITLE

PDF::API2::TTF::GPOS - Support for Opentype GPOS tables in conjunction with TTOpen
...
structure of a GPOS is shared with a GSUB table via the L<PDF::API2::TTF::Ttopen>
...
sub out_sub
{
    ...
    if ($type == 1 && $fmt == 1)
    {
        $out = pack('n2', $fmt, PDF::API2::TTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 2));
    ...
    } elsif ($type == 1 && $fmt == 2)
    {
        ...
        $out = pack('n4', $fmt, PDF::API2::TTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 2),
                            $vfmt, $#{$lookup->{'RULES'}} + 1);
        foreach $r (@{$lookup->{'RULES'}})
    ...
    } elsif ($type == 2 && $fmt < 3)
    {
        ...
        if ($fmt == 1)
        {
            $out = pack('n5', $fmt, PDF::API2::TTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 2),
                            $vfmt, $vfmt2, $#{$lookup->{'RULES'}} + 1);
        } else
        {
            $out = pack('n8', $fmt, PDF::API2::TTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 2),
                            $vfmt, $vfmt2,
                            PDF::API2::TTF::Ttopen::ref_cache($lookup->{'CLASS'}, $ctables, 1),
                            PDF::API2::TTF::Ttopen::ref_cache($lookup->{'MATCH'}[0], $ctables, 1),
    ...
    {
        $out = pack('n3', $fmt, PDF::API2::TTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 2),
                            $#{$lookup->{'RULES'}} + 1);
        foreach $r (@{$lookup->{'RULES'}})
        {
            $out .= pack('n2', PDF::API2::TTF::Ttopen::ref_cache($r->[0]{'ACTION'}[0], $ctables, length($out)),
                            PDF::API2::TTF::Ttopen::ref_cache($r->[0]{'ACTION'}[1], $ctables, length($out) + 2));
        }
    } elsif ($type == 4 || $type == 5 || $type == 6)
    {
        my ($loc_off, $loc_t, $ltables);
        
        $out = pack('n7', $fmt, PDF::API2::TTF::Ttopen::ref_cache($lookup->{'MATCH'}[0], $ctables, 2),
                            PDF::API2::TTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 4),
                            $#{$lookup->{'RULES'}[0][0]{'ACTION'}} + 1, 12, ($#{$lookup->{'MARKS'}} + 4) << 2,
                            $#{$lookup->{'MARKS'}} + 1);
        foreach $r (@{$lookup->{'MARKS'}})
        { $out .= pack('n2', $r->[0], PDF::API2::TTF::Ttopen::ref_cache($r->[1], $mtables, length($out) + 2)); }
        ...
        for ($i = 0; $i <= $#{$lookup->{'RULES'}}; $i++)
        {
            ...
            {
                foreach $s (@{$t->{'ACTION'}})
                { $out .= pack('n', PDF::API2::TTF::Ttopen::ref_cache($s, $ltables, length($out))); }
            }
    ...
    PDF::API2::TTF::Ttopen::out_final($fh, $out, \@reftables);
    $lookup;
}
...
sub read_delta
{
    ...
    $res = PDF::API2::TTF::Delta->new->read($fh);
    $fh->seek($loc, 0);
...
sub read_anchor
{
    ...
    return $lookup->{' CACHE'}{$str} if defined $lookup->{' CACHE'}{$str};
    $fh->seek($base + $offset, 0);
    $res = PDF::API2::TTF::Anchor->new->read($fh);
    $fh->seek($loc, 0);
    $lookup->{' CACHE'}{$str} = $res;
    return $res;
}
...
sub out_value
{
    ...
    foreach $n (qw(XPlaDevice YPlaDevice XAdvDevice YAdvDevice))
    {
        if ($flag & $fmt)
        {
            $out .= pack('n', PDF::API2::TTF::Ttopen::ref_cache(
...
Martin Hosken Martin_Hosken@sil.org. See L<PDF::API2::TTF::Font> for copyright and
licensing.

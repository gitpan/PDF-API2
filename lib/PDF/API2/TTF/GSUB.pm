package PDF::API2::TTF::GSUB;

=head1 NAME

PDF::API2::TTF::GSUB - Module support for the GSUB table in conjunction with TTOpen
...
The structure of a GSUB table is the same as that given in L<PDF::API2::TTF::Ttopen>.
...
sub out_sub
{
    ...
    if ($type == 1)
    {
        $out = pack("nn", $fmt, PDF::API2::TTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 2));
        if ($fmt == 1)
    ...
    PDF::API2::TTF::Ttopen::out_final($fh, $out, [[$ctables, 0]]);
    $lookup;
}

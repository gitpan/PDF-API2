package PDF::API2::TTF::Table;

=head1 NAME

PDF::API2::TTF::Table - Superclass for tables and used for tables we don't have a class for
...
The L<PDF::API2::TTF::Font> that table is part of
...
=head2 PDF::API2::TTF::Table->new(%parms)
...
sub out_xml
{
    ...
    {
        $self->read_dat;
        PDF::API2::TTF::Utils::XML_hexdump($context, $depth, $self->{' dat'});
    }
    else
    {
        $self->read;
...
'C<PDF::API2::TTF::Font-E<GT>release>' (so you don't have to call it yourself).
...
C<PDF::API2::TTF::Table> object; it'll have B<no> internal state whatsoever.

B<Developer note:>  As part of the brute-force cleanup done here, this method
will throw a warning message whenever unexpected key values are found within
the C<PDF::API2::TTF::Table> object.  This is done to help ensure that any

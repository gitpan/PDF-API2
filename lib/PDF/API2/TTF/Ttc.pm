package PDF::API2::TTF::Ttc;

=head1 NAME

PDF::API2::TTF::Ttc - Truetype Collection class
...
An array of directories (PDF::API2::TTF::Font objects) for each sub-font in the directory
...
=head2 PDF::API2::TTF::Ttc->open($fname)
...
sub read
{
    ...
    for ($i = 0; $i < $num; $i++)
    {
        ...
        $self->{'directs'}[$i] = PDF::API2::TTF::Font->new('INFILE' => $fh,
                                                'PARENT' => $self,

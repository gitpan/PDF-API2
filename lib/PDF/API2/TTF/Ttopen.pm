package PDF::API2::TTF::Ttopen;

=head1 NAME

PDF::API2::TTF::ttopen - Opentype superclass for standard Opentype lookup based tables
(GSUB and GPOS)

=head1 DESCRIPTION

Handles all the script, lang, feature, lookup stuff for a
L<PDF::API2::TTF::Gsub>/L<PDF::API2::TTF::Gpos> table leaving the class specifics to the
subclass
...
sub read_cover
{
    ...
    $cover = PDF::API2::TTF::Coverage->new($is_cover)->read($fh);
    $fh->seek($loc, 0);
...
sub out_context
{
    ...
    if (($type == 4 || $type == 5 || $type == 6) && ($fmt == 1 || $fmt == 2))
    {
        ...
        if ($fmt == 1)
        {
            $out = pack("nnn", $fmt, PDF::API2::TTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 2),
                            $num);
            $base_off = 6;
        } elsif ($type == 5)
        {
            $out = pack("nnnn", $fmt, PDF::API2::TTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 2),
                            PDF::API2::TTF::Ttopen::ref_cache($lookup->{'CLASS'}, $ctables, 4), $num);
            $base_off = 8;
        } elsif ($type == 6)
        {
            $out = pack("n6", $fmt, PDF::API2::TTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 2),
                                PDF::API2::TTF::Ttopen::ref_cache($lookup->{'PRE_CLASS'}, $ctables, 4),
                                PDF::API2::TTF::Ttopen::ref_cache($lookup->{'CLASS'}, $ctables, 6),
                                PDF::API2::TTF::Ttopen::ref_cache($lookup->{'POST_CLASS'}, $ctables, 8),
                                $num);
            $base_off = 12;
        }
    ...
    } elsif ($type == 5 && $fmt == 3)
    {
        ...
        { $out .= pack('n', PDF::API2::TTF::Ttopen::ref_cache($t, $ctables, length($out))); }
    ...
    } elsif ($type == 6 && $fmt == 3)
    {
        ...
        foreach $t (@{$r->{'PRE'}})
        { $out .= PDF::API2::TTF::Ttopen::ref_cache($t, $ctables, length($out)); }

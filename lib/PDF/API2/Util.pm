...
BEGIN {
    ...
    use vars qw($VERSION @ISA @EXPORT @EXPORT_OK %colors $key_var %u2n %n2u %u2n_o %n2u_o $pua);
    use Math::Trig;
...
sub namecolor {
    ...
    } else { # or it is a name ?
        return(floats5(@{$colors{$name} || [0.5,0.5,0.5]}));
    }
}
sub namecolor_cmyk {
    my $name=lc(shift @_);
    ...
    if($name=~/^#/) { # rgb spec.
        ...
        return(floats5(RGBasCMYK(@{$colors{$name} || [0.5,0.5,0.5]})));
    }
}
sub namecolor_lab {
    my $name=lc(shift @_);
    ...
    if($name=~/^\$/) { # lab spec.
        ...
        my ($h,$s,$v)=RGBtoHSV(@{$colors{$name} || [0.5,0.5,0.5]});
        my $a=cos(deg2rad $h)*$s*100;
        my $b=sin(deg2rad $h)*$s*100;
        my $l=100*$v;
        return(floats5($l,$a,$b));
